...
응답 형식:
1. 📊 해당 시간대와 장소, 요일, 범죄유형 수치가 제일 높은 범죄 발생 현황 (구체적인 수치 포함)
2. ⚠️ 위험도 평가: 낮음 / 보통 / 높음
3. 🛡️ 실용적인 안전 수칙 2~3가지

추가 안내:
- 시간대 데이터와 장소데이터는 2024년 기준입니다.
...
def build_crime_context():
    # 파일 경로가 맞는지 확인 필요 (FileNotFoundError 방지용 예외처리는 생략함)
    ...
    lines = ["[시간대별 총 범죄 건수 - 2024년]"]
    for lab, cnt in zip(TIME_LAB, time_total):
        lines.append(f"  {lab}: {int(cnt):,}건")
    lines.append("\n[요일별 총 범죄 건수 - 2024년]")
    for day, cnt in zip(DAY_COLM, day_total):
        lines.append(f"  {day}요일: {int(cnt):,}건")
    lines.append("\n[장소별 총 범죄 건수 TOP 20 - 2024년]")
    for place, cnt in place_total.head(20).items():
        lines.append(f"  {place}: {int(cnt):,}건")
